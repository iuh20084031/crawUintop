import scrapy
from crawunitop.items import CrawunitopItem
...
class MyscraperSpider(scrapy.Spider):
    ...
    def parseCourseDetailPage(self, response):
        ...
        item['votenumber'] = response.xpath('normalize-space(string(//*[@id="intro-head"]/div/div/div/div/div[1]/span))').get()  # số lượng người đánh giá
        stars = response.xpath('//*[@id="intro-head"]//i[@class="fas fa-star"]')
        number_of_stars = len(stars)
        item['rating'] = number_of_stars  # điểm đnáh giá
        item['newfee'] = response.xpath('normalize-space(string(//*[@id="reg-course"]/div/div[1]/span[1]))').get() # giá mới
        item['oldfee'] = response.xpath('normalize-space(string(//*[@id="reg-course"]/div/div[1]/span[2]))').get() # giá cũ
        item['lessonnum'] = response.xpath('normalize-space(string(//*[@id="course-includes"]/li[1]))').get() # số lượng bài học
        yield item